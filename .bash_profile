# Load git-aware-prompt so i know which branch i'm on and if its dirty
export GITAWAREPROMPT=~/.bash/git-aware-prompt
source $GITAWAREPROMPT/main.sh
# Fancify my command line so its colorful
export PS1="\[\e[0;32m\][\uders] \[\e[0;33m\]\w \[$txtcyn\]\$git_branch\[$txtred\]\$git_dirty\[$txtrst\]\$ "
export PATH=/usr/local/rvm/gems/ruby-2.1.1/bin:/Applications/Postgres.app/Contents/Versions/9.3/bin:./node_modules/.bin:~/bin:/usr/local/bin:/usr/local/git/bin:/usr/ctags:$PATH
subl='/Applications/Sublime Text 2.app/Contents/SharedSupport/bin/subl'
test -x "$subl" && export EDITOR="'$subl' --wait"
alias z="zeus"
alias server="python -m SimpleHTTPServer"

# Runs bash auto-complete script if loaded
if [ -f ~/.git-completion.bash ]; then
  . ~/.git-completion.bash
fi
